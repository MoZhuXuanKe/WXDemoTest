...
- (void)setDisabled:(BOOL)b{
    if (NO == b) {
        self.enabled = YES;
        self.backgroundColor =  [AppStyleConfiguration ColorWithMain];
    }else{
        self.enabled = NO;
 ...
 *  @param space titleLabel和imageView的间距
 */
- (void)layoutButtonWithMoreEdgeInsetsStyle:(MKButtonEdgeInsetsStyle)style
                        imageTitleSpace:(CGFloat)space
{
    // 1. 得到imageView和titleLabel的宽、高
    CGFloat imageWith = self.imageView.frame.size.width;
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue >= 8.0) {
        // 由于iOS8中titleLabel的size为0，用下面的这种设置
        labelWidth = self.titleLabel.intrinsicContentSize.width>self.width?self.width:self.titleLabel.intrinsicContentSize.width;
        labelWidth = self.titleLabel.intrinsicContentSize.width>self.width ?labelWidth - imageWith :labelWidth;
        labelHeight = self.titleLabel.intrinsicContentSize.height;
    } else {
        labelWidth = self.titleLabel.frame.size.width>self.width?self.width:self.titleLabel.frame.size.width;
        labelHeight = self.titleLabel.frame.size.height;
        NSLog(@"===%f",self.width);
    }
